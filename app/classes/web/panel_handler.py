...
class PanelHandler(BaseHandler):
    ...
    async def get(self, page):
        ...
        if page == "unauthorized":
            ...
            if subpage == "mods":
                bucket_data = self.controller.big_bucket.get_bucket_data()
                versions = set()
                if bucket_data:
                    for cat_key, cat_val in bucket_data.get("categories", {}).items():
                        for type_key, type_val in cat_val.get("types", {}).items():
                            for ver in type_val.get("versions", {}).keys():
                                versions.add(ver)
                page_data["available_versions"] = sorted(versions, key=lambda x: [int(n) if n.isdigit() else n for n in x.replace("v", "").split(".")], reverse=True)
